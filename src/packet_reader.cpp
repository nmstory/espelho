...
std::vector<std::unique_ptr<Replicable>> PacketReader::read(
    const uint8_t* data,
    size_t len,
    ...
    std::vector<std::unique_ptr<Replicable>>& objects)
{
  ...
  while (stream.pos < stream.cap) {
    TypeID tid;
    if (!process_pod(stream, tid)) {
      break;
    }

    int id;
    if (!process_pod(stream, id)) {
      break;
    }
    ...
    if (objItr != objects.end()) {
      if (!(*objItr)->process(stream)) {
        break;  // deserialise in-place
      }
    } else {
